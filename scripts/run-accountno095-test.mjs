#!/usr/bin/env node
/**
 * End-to-end test: accountno095details → Premium project → compare with manual BRS.
 * Usage: API_URL=http://localhost:9011 node scripts/run-accountno095-test.mjs
 */
import fs from 'fs'
...
/** Ecobank xlsx after API normalization: Date, Description, Ref, Value Date, Debit(4), Credit(5), Balance(6) */
...
async function main() {
  ...
  if (!proj.documents?.length) {
    const cb = path.join(DATA, 'LIBcashbk2 2026 1qtr.xlsx')
    const bank = path.join(DATA, '1778676142095 dated 4.6.26.xlsx')
    const acct = 'Ecobank Tesano 1441001519035'
    const acctNo = '1441001519035'
    console.log('\nUploading cash book...')
  ...
  const tieOutDelta = Math.abs(tieOut - MANUAL.intrinsicTieOutVariance)
  const tieOutOk = tieOutDelta < 1 || Math.abs(tieOut) < 0.02
  console.log(
    `  ${tieOutOk ? '✓' : '✗'} Tie-out variance: ${tieOut} (manual intrinsic ~${MANUAL.intrinsicTieOutVariance})`
  )
  const matchedCount = report.summary?.matchedCount ?? report.matchedCount ?? totalMatched
  const matchNote =
    matchedCount === MANUAL.matchedPairs ? '✓' : 'ℹ'
  console.log(
    `  ${matchNote} Matched pairs: ${matchedCount} (manual workbook ~${MANUAL.matchedPairs}; use SAFE_MATCH_ONLY=1 for conservative bulk-match)`
  )
  console.log(`\nProject slug: ${project.slug}`)
  console.log(`Web UI: http://localhost:9100 (log in as ${EMAIL})`)
