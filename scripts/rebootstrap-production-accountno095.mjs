#!/usr/bin/env node
/**
 * Rebuild a production (or any) Ecobank 9035 project from accountno095details xlsx.
 * Deletes existing slug, recreates, uploads, maps, sets BRS notes, clears matches, auto-matches.
 *
 * Usage:
 *   API_URL=https://api.kqsoftwaresolutions.com node scripts/rebootstrap-production-accountno095.mjs
 */
import fs from 'fs'
...
const PROJECT_SLUG = process.env.BRS_PROJECT_SLUG || 'lordship-ecobank-9035-q1-2026'
const PROJECT_NAME = process.env.BRS_PROJECT_NAME || 'Lordship – Ecobank 9035 Q1 2026'
const RECON_DATE = '2026-03-31T00:00:00.000Z'
const BANK_CLOSING = 4899.28
...
async function main() {
  ...
  for (const f of ['LIBcashbk2 2026 1qtr.xlsx', '1778676142095 dated 4.6.26.xlsx']) {
    if (!fs.existsSync(path.join(DATA, f))) throw new Error(`Missing ${f} in accountno095details/`)
  }
  ...
  const project = await api('POST', '/projects', token, {
    ...
    primaryAccountNo: '1441001519035',
  })
  ...
  console.log(`\nMatched pairs: ${report.matchedPairs?.length ?? report.summary?.matchedCount ?? totalMatched}`)
